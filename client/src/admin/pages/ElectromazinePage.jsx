...
const ElectromazinePage = () => {
    ...
    const handleAddSubmit = async (e) => {
        ...
        if (!imageFile || !description || !registrationLink) {
            setFormError('Please fill out all fields and select an image.');
        ...
        try {
            await apiClient.post(`${magazineApiUrl}/upload`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });

            setFormSuccess('Electromazine added successfully!');
            setImageFile(null);
            setImagePreviewUrl(null);
            setDescription('');
            setRegistrationLink('');
        } catch (err) {
            setFormError(err.response?.data?.message || 'Failed to add electromazine.');
            console.error('Add electromazine error:', err);
        } finally {
            ...
            <div className={`container max-w-md md:max-w-xl lg:max-w-2xl mx-auto ${formCardBg} rounded-3xl shadow-xl ${neutralBorder} p-6 sm:p-8 lg:p-10 my-6 sm:my-8 z-10`}>
                <div className={`p-6 rounded-2xl ${neutralBorder} mb-8 ${innerFormSectionBg} transition duration-300 ease-in-out hover:shadow-lg`}>
                    <h2 className={`${generalDarkText} text-3xl font-extrabold mb-5 border-b ${neutralBorder} pb-3 flex items-center ${bodyFontClass}`}>
                        <svg className={`h-7 w-7 ${iconColor} mr-3`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6.253v11.494m-5.747-5.747h11.494" /></svg>
                        New Electromazine Details
                    </h2>

                    {formError && (
                        <div className="bg-red-50 border border-red-200 text-red-7 p-3 rounded-lg flex items-center mb-5 text-sm">
                    ...
                    <form onSubmit={handleAddSubmit} className="space-y-6">
                        ...
                        <div>
                            <label htmlFor="description" className={`block ${generalDarkText} text-sm font-semibold mb-2 ${bodyFontClass}`}>
                                Description:
                            </label>
                            <textarea
                                id="description"
                                rows="4"
                                className={`shadow-sm appearance-none ${neutralBorder} rounded-xl w-full py-2.5 px-4 ${generalDarkText} leading-tight focus:outline-none ${inputFocusRing} focus:border-transparent resize-y ${bodyFontClass}`}
                                placeholder="Enter a short description for the electromazine..."
                                value={description}
                        ...
                        <div>
                            <label htmlFor="registrationLink" className={`block ${generalDarkText} text-sm font-semibold mb-2 ${bodyFontClass}`}>
                                Registration Link:
                            </label>
                            <input
                                type="url"
                                ...
                                placeholder="e.g., https://forms.gle/your-registration-link"
                        ...
                        >
                            {isLoading ? (
                                <>
                                    ...
                                    Adding Electromazine...
                                </>
                            ) : (
                                <>
                                    <svg className="h-5 w-5 mr-2 -ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"></path></svg>
                                    Add Electromazine
                                </>
                            )}
                        </button>
                ...
                <div className="text-center mt-8">
                    ...
                    >
                        View & Manage Electromazines
                        <svg className={`ml-2 w-5 h-5 ${iconColor}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                    </Link>
                </div>
            </div>
        </div>
...
export default ElectromazinePage;